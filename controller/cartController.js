...
exports.addItem = (req, res, next) => {
  ...
  db.query("INSERT INTO cart(user_id, product_id) values (?, ?)", [req.currentUser.id, req.body.product_id], (err, rows) => {
    if(err) {
      console.log(err);
...
exports.getItemsByUser = (req, res, next) => {
  ...
  db.query("SELECT cart.id as id, product_id, user_id, cover_image, category, price, discount, name FROM cart JOIN products on cart.product_id = products.id WHERE cart.user_id = ? ORDER BY price DESC",[req.params.user_id], (err, rows) => {
    if(err) {
       res.json({
